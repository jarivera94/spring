...
@Service("estrategiaCreados")
public class EstrategiaAvaluosCreados extends EstrategiaAvaluo implements EstrategiaConsultaAvaluo {
    ...
    public long contarAvaluosPorFiltrosOrden(int primero,
        ...
        Predicate numeroDocumentoCreador = criteriaBuilder.equal(
                creador.get(Usuario_.id).get("numeroDocumento"),
                usuario.getNumeroDocumento());
        Predicate tipoDocumentoCreador = criteriaBuilder.equal(
                creador.get(Usuario_.id)
                .get("tipoDocumentoIdentificacion"),
                usuario.getTipoDocumentoIdentificacion());
        Predicate numeroDocumentoCoordinador = criteriaBuilder.equal(
                creador.get(Usuario_.numeroDocumentoCoordinadorAsignado),
                usuario.getNumeroDocumento());
        Predicate tipoDocumentoCoordinador = criteriaBuilder.equal(
                creador.get(Usuario_.tipoDocumentoCoordinadorAsignado),
                usuario.getTipoDocumentoIdentificacion());
        consultaContar.where(criteriaBuilder
                .and(estadoActual,filtro,entidadesUsuario,
                    tiposAvaluoUsuario,
                    criteriaBuilder.or(
                        criteriaBuilder.and(tipoDocumentoCreador,
                            numeroDocumentoCreador),
                        criteriaBuilder.and(tipoDocumentoCoordinador,
    ...
    public List<AvaluoDTO> encontrarAvaluosPorFiltrosOrden(int primero,
        ...
        Root<EstadoAvaluo> estadoAvaluo = listadoQuery.from(
                EstadoAvaluo.class);
        Join<EstadoAvaluo, Avaluo> avaluo = estadoAvaluo
            .join(EstadoAvaluo_.avaluo);
        Join<Avaluo, Entidad> entidad = avaluo.join(Avaluo_.entidad);
        ...
        Join<Avaluo, TipoAvaluo> tipoAvaluo = avaluo
            .join(Avaluo_.tipoAvaluo);
        Join<Avaluo, Usuario> perito = avaluo.join(Avaluo_.perito,
                JoinType.LEFT);
        Join<Avaluo, Usuario> creador = avaluo.join(Avaluo_.creador,
                JoinType.LEFT);
        ...
        log.info("Consulta estrategia creador: {}",
