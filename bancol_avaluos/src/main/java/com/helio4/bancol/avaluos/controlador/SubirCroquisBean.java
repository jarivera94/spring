...
@Controller
@Scope("view")
@Qualifier("subirCroquisBean")
public class SubirCroquisBean implements Externalizable {

    private static Logger log = LoggerFactory.getLogger(SubirCroquisBean.class);
    ...
    @PostConstruct
    public void init() {
        IMAGES_PATH = parametrosController.obtenerValor("rutaCroquis");
        avaluo = listadoAvaluosBean.getAvaluo();
        fotografias = fotografiasController.buscarCroquisAvaluo(avaluo);
    ...
    private Panel crearPanelFotografia(int indice, FotografiaDTO fotografiaDTO) {
        ...
        MethodExpression methodExpression = expressionFactory
                .createMethodExpression(fc.getELContext(),
                        "#{subirCroquisBean.eliminarFotografia}", null,
    ...
    public String crearURLFotografia(Long id, String rutaAnterior) {
        return urlSeparator + "croquis" + urlSeparator
    ...
    public void subirFoto(FileUploadEvent event) {
    	orden++;
    	if (orden < MAX_NUM_FILES) {
            fotografiaDTO = fotografiasController.subirCroquis(event, IMAGES_PATH,
            ...
            if(avaluo.getEstado().getEstado().key()
        	 ...
        	 FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Se ha alcanzado el límite de carga de archivos", event.getFile().getFileName() + " no se ha subido.");
    ...
    public void actualizarDescripcion(AjaxBehaviorEvent event) {
        try {
            ...
            fotografiasController.actualizarCroquis(fotografia);
    ...
    public void eliminarFotografia(ActionEvent event) {
        ...
        try {
            ...
            fotografiasController.eliminarCroquis(fotoEliminada.getId());
    ...
    public void cancelar() {
        ...
        String uri = fc.getExternalContext().encodeActionURL(
                FacesContext.getCurrentInstance().getExternalContext().getRequestContextPath()+listadoAvaluosBean.getUrlReturn());
        try {
    ...
    public void handleReorder(DashboardReorderEvent event) {
        ...
        try {
            ...
            fotografias = fotografiasController
                .moverCroquis(fotoMovida, posicionActual,
                    indiceNuevo, fotografias);
            log.info("croquis {}", fotografias);
        } catch (NumberFormatException e) {
